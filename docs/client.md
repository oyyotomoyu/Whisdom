...
Whisdom uses a React frontend. The main user experience is a conversation workspace similar to common GPT applications: users open the app, see recent conversations, select or start a chat, and ask questions against the approved library.

Admin-only tools are available only to authorized users. Regular users should not see material upload, model teaching, or training path configuration in the client.

---

# Reference Pattern

The client structure should follow the important frontend pattern used in the NexGestion project:
...
The client should feel like a private company GPT app:
...
Admin-only client areas include:
...
Admins can add library resources and training materials from multiple
sources. Uploading a file is one supported source, but it must not be the only
way to provide material.

The material source interface should support:

* Drag-and-drop upload
* File picker
* Existing server/local paths
* Internal wiki URLs
* Git repository URLs, such as GitLab-hosted documentation
...
* Audio
* Wiki pages
* Git-hosted Markdown or documentation
...
## Material Source And Destination Editing

Admins can edit where material comes from and where it is sent for Whisdom
processing.

The source location represents the original material location. It may be an
uploaded file, backend/local path, internal wiki URL, GitLab URL, or another
configured organization source.

The destination path represents the internal model training or knowledge
processing destination used by Whisdom after the source material is collected.
...
```text
/mnt/company/docs/policy/
https://wiki.company.local/hr/policy
https://gitlab.company.local/docs/support-handbook
```

The client should validate destination paths and source locations before saving.
...
* Source location is required unless the material is provided by direct upload.
* Local/backend paths must block traversal.
* URLs must use an allowed scheme, such as `https://` or an explicitly
  configured internal scheme.
* External sources must be accepted only when the organization has configured
  the required connector, allowlist, and access permissions.
...
The correction interface should allow an admin to record:
...
* Dev-mode client behavior belongs in request modules or auth providers, guarded by `process.env.NODE_ENV !== "production"`.
...
All visible client text should come from translation keys, including:
...
The client must support mobile and PC.
