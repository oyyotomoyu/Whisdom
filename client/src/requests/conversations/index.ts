import { API_BASE_URL, getAccessToken, request } from "../core";
...
export async function sendMessage(conversationId: string, message: string): Promise<SendMessageResult> {
  const data = await request<{ message_id: string; answer: string; sources: MessageSource[] }>(
...
export async function streamMessage(
  ...
  try {
    ...
    if (!response.headers.get("Content-Type")?.includes("text/event-stream")) {
      const data = (await response.json()) as { answer: string; sources: MessageSource[] };
